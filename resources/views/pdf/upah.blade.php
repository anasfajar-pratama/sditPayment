...
<head>
    ...
    <style>
        ...
        .section-title { font-size: 9pt; font-weight: 700; margin: 8px 0 4px; padding: 3px 6px; background: #eef2ff; border-left: 3px solid #6366f1; }
...
<div class="header">
    <h1>LAPORAN PENGELUARAN UPAH</h1>
...
@foreach ($penerimas as $p)
    @if (!empty($grouped[$p]))
        ...
        <table>
            ...
            <tfoot>
                <tr class="total-row">
                    <td colspan="3" style="text-align:right;">Total {{ $p }}</td>
                    <td class="right" style="color:#dc2626;">{{ number_format($ringkasan[$p], 0, ',', '.') }}</td>
                    <td></td>
                </tr>
            </tfoot>
        </table>
...
<div class="footer">
    Laporan Pengeluaran Upah — {{ \Carbon\Carbon::parse($start)->format('d/m/Y') }} s.d. {{ \Carbon\Carbon::parse($end)->format('d/m/Y') }}
