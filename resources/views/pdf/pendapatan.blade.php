...
<head>
    ...
    <style>
        ...
        .section-title { font-size: 9pt; font-weight: 700; margin: 8px 0 4px; padding: 3px 6px; background: #fffbeb; border-left: 3px solid #d97706; }
...
@foreach ($akunList as $akun)
    ...
    <div class="section-title">{{ $akun['kode'] }} — {{ $akun['nama'] }}</div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>URAIAN</th>
                <th style="width:9rem;">REKENING TUJUAN</th>
                <th style="width:8rem;" class="right">JUMLAH (Rp)</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($grouped[$akun['id']] as $row)
                <tr>
                    <td class="center" style="color:#9ca3af;">{{ $row['no'] }}</td>
                    <td style="color:#6b7280;">{{ $row['tanggal'] }}</td>
                    <td>{{ $row['uraian'] }}</td>
                    <td>
                        {{ $row['rekening'] ?: 'Cash' }}{{ $row['pengirim'] ? ' (' . $row['pengirim'] . ')' : '' }}
                    </td>
                    <td class="right" style="color:#047857;">{{ number_format($row['jumlah'], 0, ',', '.') }}</td>
                </tr>
            @empty
                <tr><td colspan="5" style="text-align:center;color:#9ca3af;">Tidak ada transaksi</td></tr>
            @endforelse
        </tbody>
        <tfoot>
            <tr class="total-row">
                <td colspan="4" style="text-align:right;">Total {{ $akun['kode'] }} — {{ $akun['nama'] }}</td>
                <td class="right" style="color:#047857;">{{ number_format($ringkasan[$akun['id']], 0, ',', '.') }}</td>
            </tr>
        </tfoot>
    </table>
@endforeach
...
<div class="footer">
    Laporan Pendapatan — {{ \Carbon\Carbon::parse($start)->format('d/m/Y') }} s.d. {{ \Carbon\Carbon::parse($end)->format('d/m/Y') }}
